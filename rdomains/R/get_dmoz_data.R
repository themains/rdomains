#' Get DMOZ Data
#'
#' Downloads, unzips and saves the latest version of the DMOZ data.
#'  
...
#' @export
#' 
#' @references \url{http://rdf.dmoz.org/}
#' 
...
get_dmoz_data <- function(outdir="./", overwrite=FALSE) {
	...
	tmp <- tempfile()
	curl_download("http://rdf.dmoz.org/rdf/content.rdf.u8.gz", tmp)
	read_file(gzfile(tmp))
	unlink(tmp)
